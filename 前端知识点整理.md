...
- 当多个属性影响一个属性时，建议使用`computed`；
- 当一个值发生变化，便会引起一系列操作（如改变其他属性值），建议使用`watch`；
- 如果一个数据需要经过复杂计算就使用`computed`；
- 如果在数据变化时执行异步或开销较大的操作就用`watch`。
...
> 通常在**created**函数中发送ajax请求获取数据，在**mounted**中获取挂载完毕后真实的DOM，**destroy**中销毁定时器、延时器或绑定的一些事件。

### 1.父子组件生命周期

周期运行顺序：

父`beforeCreate` -> 父`created` -> 父`beforeMount` -> 子`beforeCreate` -> 子`created` -> 子`beforeMount` -> 子的`mounted` -> 父的`mounted`
